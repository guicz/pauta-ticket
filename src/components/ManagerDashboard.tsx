            ...
            <tbody>
              {visibleTasks.map((task) => (
                <tr key={task.id} className={`demand-priority-${task.priority}`}>
                  ...
                  <td data-label="Responsável"><select className="inline-select" aria-label={`Responsável por ${task.title}`} value={task.assignee} disabled={task.status === "completed"} onChange={event => onChangeAssignee(task.id, event.target.value as "pati" | "gui")}><option value="gui">Gui</option><option value="pati">Pati (eu)</option></select></td>
                  <td data-label="Estimativa"><strong>{formatMinutes(task.executorEstimateMinutes ?? task.estimatedMinutes)}</strong>{task.executorEstimateMinutes && <small>ajustada pelo Gui</small>}</td>
                  <td data-label="Agendamento" className="planning-cell"><button className={`schedule-trigger ${task.scheduledStart || task.shift ? "has-schedule" : ""}`} onClick={() => setScheduling(task)} aria-label={`Editar horário de ${task.title}`}><CalendarClock size={18} /><span><strong>{scheduleTimeLabel(task)}</strong><small>{scheduleMetaLabel(task)}</small></span><span className="schedule-edit">Editar</span></button><select className="inline-select recurrence-select" aria-label={`Repetição de ${task.title}`} value={task.recurrence ?? "none"} onChange={(event) => onUpdateTask(task.id, { recurrence: event.target.value as Task["recurrence"] })}><option value="none">Não repetir</option><option value="daily">Diária</option><option value="weekly">Semanal</option><option value="monthly">Mensal</option></select></td>
                  ...
                  <td data-label="Estado"><span className={`status status-${task.status}`}>{statusLabel(task.status)}</span>{overdueTask(task, state.events, clock) && <span className="overdue-badge" title="O horário previsto terminou e a demanda ainda não foi enviada para avaliação ou concluída.">Atrasada</span>}</td>
